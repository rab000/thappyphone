...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerParticleData : PhysicsManagerAccess
    {
        ...
        public override void Create()
        {
            ...
            radiusList = new FixedChunkNativeArray<float>();
        ...
        public ChunkData CreateParticle(
            ...
            System.Func<int, float> funcRadius,
        ...
        struct CopyBoneToParticleJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                if (moveInfluence < 0.99f || rotInfluence < 0.99f)
                {
                    ...
                    quaternion orot = math.slerp(quaternion.identity, wdata.rotationOffset, (1.0f - rotInfluence));
                    float3 opos = wdata.moveOffset * (1.0f - moveInfluence);

                    var pos = oldPosList[index];
                    var lpos = pos - wdata.oldPosition;
                    lpos = math.mul(orot, lpos);
                    var npos = wdata.oldPosition + lpos;
                    npos += opos;
                    var offset = npos - pos;

                    oldPosList[index] = oldPosList[index] + offset;
                ...
                {
                    bpos = bpos + math.mul(brot, localPosList[index]);
                }
