...
namespace NLog.UI
{
    public delegate void CellViewVisibilityChangedDelegate(LoopScrollerCellView cellView);
    public delegate void CellViewWillRecycleDelegate(LoopScrollerCellView cellView);
    public delegate void ScrollerScrolledDelegate(LoopScroller scroller, Vector2 val, float scrollPosition);
    public delegate void ScrollerSnappedDelegate(LoopScroller scroller, int cellIndex, int dataIndex, LoopScrollerCellView cellView);
    public delegate void ScrollerScrollingChangedDelegate(LoopScroller scroller, bool scrolling);

    /// <summary>
    /// 处理滚动状态改变 (jumping 或者 not jumping)
    /// </summary>
    public delegate void ScrollerTweeningChangedDelegate(LoopScroller scroller, bool tweening);

    /// <summary>
    /// 单元格首次创建回调
    /// </summary>
    public delegate void CellViewInstantiated(LoopScroller scroller, LoopScrollerCellView cellView);

    /// <summary>
    /// 单元格重用回调
    /// </summary>
    public delegate void CellViewReused(LoopScroller scroller, LoopScrollerCellView cellView);
    ...
    public class LoopScroller : MonoBehaviour
    {
        ...
        /// </summary>
        public ILoopScrollerDelegate Delegate { get { return _delegate; } set { _delegate = value; _reloadData = true; } }
        ...
        /// <returns></returns>
        public LoopScrollerCellView GetCellView(LoopScrollerCellView cellPrefab)
        {
            ...
            if (cellView == null)
            {
                ...
                cellView = go.GetComponent<LoopScrollerCellView>();
            ...
            )
        {
            ...
            if (!Mathf.Approximately(cellOffset, 0))
            {
        ...
        /// <returns></returns>
        public LoopScrollerCellView GetCellViewAtDataIndex(int dataIndex)
        {
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
        ...
        /// </summary>
        private ILoopScrollerDelegate _delegate;
        ...
        /// </summary>
        private readonly BetterList<LoopScrollerCellView> _recycledCellViews = new BetterList<LoopScrollerCellView>();
        ...
        /// </summary>
        private readonly BetterList<float> _cellViewSizeArray = new BetterList<float>();
        ...
        /// </summary>
        private readonly BetterList<float> _cellViewOffsetArray = new BetterList<float>();
        ...
        /// </summary>
        private readonly BetterList<LoopScrollerCellView> _activeCellViews = new BetterList<LoopScrollerCellView>();
        ...
        /// </summary>
        /// <param name="scrollerSpacing">new spacing value</param>
        private void UpdateSpacing(float scrollerSpacing)
        {
            _updateSpacing = false;
            _layoutGroup.spacing = scrollerSpacing;
        ...
        /// <returns></returns>
        private LoopScrollerCellView GetRecycledCellView(LoopScrollerCellView cellPrefab)
        {
            for (var i = 0; i < _recycledCellViews.Count; i++)
            {
        ...
        private void ResetVisibleCellViews()
        {
            ...
            var i = 0;
            BetterList<int> remainingCellIndices = new BetterList<int>();
        ...
        private void RecycleCell(LoopScrollerCellView cellView)
        {
        ...
        void Awake()
        {
            //padding.left = (int)(Screen.width / 2.64f + 0.5f);
            //if (padding.left < 500)
            //{
            //    padding.left += 500;
            //}
            //padding.right = (int)(Screen.width / 2.64f + 0.5f);
            //if (padding.right < 500)
            //{
            //    padding.right += 500;
            //}
        ...
        void Update()
        {
            ...
            if (
                    (loop && !Mathf.Approximately(_lastScrollRectSize, ScrollRectSize))
                    ||
                    (loop != _lastLoop)
                )
            {
            ...
            if (!Mathf.Approximately(LinearVelocity, 0) && !IsScrolling)
            {
                IsScrolling = true;
                scrollerScrollingChanged?.Invoke(this, true);
            }
            else if (Mathf.Approximately(LinearVelocity, 0) && IsScrolling)
            {
                IsScrolling = false;
        ...
        void OnValidate()
        {
            ...
            _updateSpacing |= _initialized && !Mathf.Approximately(spacing, _layoutGroup.spacing);
        }
        ...
        private void ScrollRect_OnValueChanged(Vector2 val)
        {
            ...
            if (snapping && !_snapJumping)
            {
                ...
                if (Mathf.Abs(LinearVelocity) <= snapVelocityThreshold && !Mathf.Approximately(LinearVelocity, 0))
                {
        ...
        private void SnapJumpComplete()
        {
            ...
            LoopScrollerCellView cellView = null;
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
        ...
        {
            if (tweenType == TweenType.immediate || Mathf.Approximately(time, 0))
            {
        ...
        private static float EaseInElastic(float start, float end, float val)
        {
            ...
            val /= d;
            if (Mathf.Approximately(val, 1))
            {
                return start + end;
            }

            float s;
            if (Mathf.Approximately(a, 0f) || a < Mathf.Abs(end))
            {
                a = end;
                s = p / 4;
            }
